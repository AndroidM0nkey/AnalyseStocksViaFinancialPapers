...
class ClientApiService(analysis_pb2_grpc.ClientApiServiceServicer):
    ...
    async def AnalyzeDocument(
        ...
        await self.redis.hset(f"analysis:{document_id}", mapping={"status": "processing"})
        ...
        await self.redis.hset(
            ...
            mapping={"status": "completed", "kpi_count": len(kpi_response.kpis)},
        )
...
async def serve() -> None:
    ...
    await mark_serving(health_servicer, ["analysestocks.v1.ClientApiService"])
